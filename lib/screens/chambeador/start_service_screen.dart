...
class _StartServiceScreenState extends State<StartServiceScreen> {
  ...
  bool _hasContractInProgress = false; // Track in-progress contract
  ...
  Future<void> _checkContractStatus() async {
    ...
    try {
      ...
      // Check contract offer
      final offerResponse = await http.get(
        Uri.parse('https://chambea.lat/api/contracts/offer/${widget.job.id}'),
        headers: {
      ...
      // Check contract status (new API call)
      final statusResponse = await http.get(
        Uri.parse('https://chambea.lat/api/contracts/status/${widget.job.id}'),
        headers: {
          'Authorization': 'Bearer $token',
          'Content-Type': 'application/json',
          'Accept': 'application/json',
      ...
      if (offerResponse.statusCode == 200 && statusResponse.statusCode == 200) {
        ...
        final hasContractInProgress = statusData['data']['has_contract_in_progress'] as bool;
        ...
        final error = json.decode(offerResponse.body)['message'] ??
  ...
  Future<void> _acceptAndStartService(BuildContext context) async {
    ...
    try {
      ...
      final acceptResponse = await http.post(
        Uri.parse('https://chambea.lat/api/service-requests/${widget.job.id}/accept'),
        headers: {
          'Authorization': 'Bearer $token',
          'Content-Type': 'application/json',
          'Accept': 'application/json',
      ...
      if (acceptResponse.statusCode != 200) {
        final error = json.decode(acceptResponse.body)['message'] ?? 'Error desconocido';
        ScaffoldMessenger.of(context).showSnackBar(
      ...
      final startResponse = await http.post(
        Uri.parse('https://chambea.lat/api/service-requests/${widget.job.id}/start'),
        headers: {
          'Authorization': 'Bearer $token',
          'Content-Type': 'application/json',
          'Accept': 'application/json',
      ...
      if (startResponse.statusCode == 200) {
        final updatedJob = Job.fromJson(json.decode(startResponse.body)['data']);
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Servicio iniciado exitosamente')),
        );
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(builder: (_) => TerminateServiceScreen(job: updatedJob)),
        );
      } else {
        final error = json.decode(startResponse.body)['message'] ?? 'Error desconocido';
        ScaffoldMessenger.of(context).showSnackBar(
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                            ...
                            children: [
                              ...
                              Row(
                                children: [
                                  ...
                                  Text(widget.job.location, style: const TextStyle(fontSize: 14)),
                                  const Spacer(),
                              ...
                              Row(
                                children: [
                                  ...
                                  Text(
                                    (widget.job.clientRating ?? 0.0).toStringAsFixed(1),
                        ...
                        Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              const SizedBox(height: 8),
                              Text(
                                'Hola ${widget.job.clientName ?? 'Usuario Desconocido'}, soy ${widget.job.workerName ?? 'Trabajador'}, técnico eléctrico. '
                        ...
                        if (_hasContractOffer && widget.job.status == 'En curso') ...[
                          SlideAction(
                            borderRadius: 12,
                            elevation: 0,
                        ...
                        if (_hasContractInProgress && widget.job.status == 'En curso') ...[
                          SlideAction(
                            borderRadius: 12,
                            elevation: 0,
                            ...
                            onSubmit: () {
                              Navigator.push(
                                context,
                                MaterialPageRoute(builder: (_) => TerminateServiceScreen(job: widget.job)),
                              );
                            },
                          ),
                        ],
