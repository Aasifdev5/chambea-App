    ...
    try {
      ...
      if (widget.proposalId != null) {
        ...
        if (selectedProposal.isNotEmpty) {
          agreedBudget =
              double.tryParse(
                selectedProposal['proposed_budget']?.toString() ?? '0',
              ) ??
              0.0;
      ...
      if (agreedBudget == 0.0) {
        agreedBudget =
            double.tryParse(data['budget']?.toString() ?? '0') ?? 0.0;
      }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('Contrato #${widget.requestId}'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
            child: const Text(
        ...
        listener: (context, state) {
          if (state is ProposalsActionSuccess) {
            ...
            if (errorMessage.contains(
              'Ya has enviado una oferta o contratado a este trabajador',
            )) {
              errorMessage =
                  'Ya has enviado una oferta o contratado a este trabajador para esta solicitud de servicio.';
            } else {
